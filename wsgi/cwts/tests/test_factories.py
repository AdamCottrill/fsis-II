...
class TestCWTfactory(TestCase):
    ...
    def setUp(self):
        ...
        self.cwt_number = 631111
        self.cwt = CWTFactory(spc=self.spc, cwt=self.cwt_number)

    def test_cwt_factory(self):

        self.assertEqual(self.cwt.stocked, True)
        self.assertEqual(self.cwt.cwt, self.cwt_number)
        ...
        self.assertEqual(self.cwt.plant_site, "Thunder Beach")
        self.assertEqual(self.cwt.ltrz, 8)
...
class TestRecoveryfactory(TestCase):
    ...
    def setUp(self):
        ...
        self.cwt_number = '631111'
        self.recovery = CWT_RecoveryFactory(cwt=self.cwt_number)

    def test_recovery_factory(self):

        self.assertEqual(self.recovery.cwt, self.cwt_number)
