...
-- given a polygon as wkt, return all of the recoveries of cwts stocked in the geom between
...
RETURN QUERY SELECT distinct cwt_recovery.*
...
JOIN fsis2_species AS spc on spc.id=cwt_recovery.spc_id
  JOIN (SELECT DISTINCT fs_event.id,
        ...
        st_intersects (fs_event.geom, st_geomfromtext(_geom,4326))) AS events
...
--  -- without speceis ids:
--  select cwt, st_astext(geom), recovery_date from cwts_recovered_geom(
--              '{"type": "Polygon", "coordinates": [[
--              [-82.271, 45.514],
--              [-82.380, 44.512],
--              [-81.128, 44.449],
--              [-81.172, 45.406],
--              [-82.271, 45.514]]]}', 1995, 2001) limit 10;
--
...
-- given geometry for a region of interest as wkt, return all of the stocking event associated with cwts recovered
...
FROM fsis2_event AS fs_event
  ...
  JOIN (SELECT cwt,
        ...
        recovery.recovery_year >= _fyear and
        recovery.recovery_year <= _lyear and
        st_intersects (recovery.geom, st_geomfromtext(_geom,4326))) AS cwts
    ON (cwts.cwt = cwts_applied.cwt
   AND cwts.spc_id = lot.species_id)
   WHERE tagging_event.tag_type=6;
