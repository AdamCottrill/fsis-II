...
class TestPasswordReset(TestCase):
    ...
    def test_superuser_password_reset_for_nonexistant_user(self):
        ...
        response = self.client.post(reverse('password_reset'), postdata)
        msg = ("<li>Sorry, this user doesn&#39;t exist.</li>")
        self.assertContains(response, msg, html=True)
        self.assertTemplateUsed(response,
                                'password_reset/recovery_form.html')

        def test_user_tries_access_invalid_link():
            """if a user tries to access an incorrect/expired reset
            link, they should be redirected to a page that notifies
            them of the problem and contains a link request a new
            one"""
        ...
        response = self.client.get(reverse('password_reset_reset',
                                           kwargs={'token': '1234'}),
                                   follow=True)

        self.assertTemplateUsed(response, 'password_reset/reset.html')
        msg = ("Sorry, this password reset link is invalid")
        self.assertContains(response, msg)
        msg = '<a href="{0}">request a new one</a>'
        msg = msg.format(reverse('password_reset'))
        self.assertContains(response, msg)
